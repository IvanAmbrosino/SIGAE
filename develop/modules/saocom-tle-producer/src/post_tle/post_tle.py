...
class PostTle():
    ...
    def __init__(self, logger: logging.Logger):
        ...
        self.satellite_condfig  = self.config_manager.config["satellite_config"]
        if self.kafka_config['topic']: # Si se especifica un topic en la configuracion, se usa ese
            self.kafka_topic    = self.kafka_config['topic']
        else: # Si no se especifica un topic, se usa el id del satelite
            self.kafka_topic    = self.satellite_condfig["satellite_id"]
        self.producer           = None
        self.logger             = logger
    ...
    def kafka_producer(self, message):
        ...
        self.producer.produce(topic =       self.kafka_config['topic'],
                              key =         self.satellite_condfig["satellite_id"],
                              value =       message,
